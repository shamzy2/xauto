...
export async function POST(req: Request) {
  ...
  const verified = verifyIntakeViewToken(
    cookieStore.get(INTAKE_VIEW_COOKIE)?.value,
  );
  ...
  if (!kjennemerke || kjennemerke.length < 2 || !kilometerstand) {
    return NextResponse.json(
      { error: "Kunne ikke koble til kjøretøy (start på nytt på /selg)." },
      { status: 400 },
    );
  }
  ...
  for (let i = 0; i < photoFiles.length; i++) {
    ...
    const objectPath = `${id}/${i}-${safeName || "photo"}`;
  ...
  const { error: insErr } = await admin.from("sell_submissions").insert({
    id,
    intake_id: intakeId,
    kjennemerke,
    kilometerstand,
  ...
  if (insErr) {
    console.error("[sell_submissions insert]", insErr);
