const DB_NAME = "xauto-sell-finalize";
const STORE_NAME = "kv";
...
export async function readFinalizePhotos(): Promise<File[]> {
...
export async function persistFinalizePhotos(files: File[]): Promise<void> {
  if (typeof indexedDB === "undefined") return;
  try {
...
export async function clearFinalizePhotos(): Promise<void> {
  await persistFinalizePhotos([]);
}
